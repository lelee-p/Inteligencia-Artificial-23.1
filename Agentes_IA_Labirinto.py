...
class AgentDFS:
    ...
    def act(self):
        ...
        plt.ion()
        while self.F:
            path = self.F.pop(-1) #retira o ultimo elemento do caminho
            ...
            if (self.percepcoes['posicao'] == self.percepcoes['saida']).all():
                ...
                for vi in self.percepcoes['vizinhos']: #poda de ciclo
                    forma_ciclo = False
                    for no in path:
                        if (no == vi).all():
                            forma_ciclo = True
...
class AgentEsperto:
    ...
    def act(self):
        ...
        while self.F:
            ...
            if self.type == 'Greedy':
                posicao_na_fronteira = np.argmin(self.H) #retorna a posicao do menor valor de self.H
            ...
            if (self.percepcoes['posicao'] == self.percepcoes['saida']).all():
                ...
                for vi in self.percepcoes['vizinhos']:
                    ...
                    if not forma_ciclo: #atualiza as listas
                        self.F.append(path + [vi])
...
def cost(path):
    return len(path)-1 #o -1 é pq o num de passos é o caminho-1

def heuristica(no, objetivo):
    manhattan_distance = np.sum(np.abs(no-objetivo)) #distancia do nó atual até o objetivo
